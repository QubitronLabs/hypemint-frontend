...
export function OnChainTradingPanel({
  ...
  const primaryWalletConnector = primaryWallet?.connector as  WalletConnector & EvmWalletConnector;
  ...
  // Quotes (EVM only — Solana doesn't have on-chain quote functions)
  ...
  const isBuyConfirming = isSolana ? isSolanaBuyConfirming : isEvmBuyConfirming;
  ...
  const isSellConfirming = isSolana
    ...
    : isEvmSellConfirming;
  const isSellConfirmed = isSolana ? isSolanaSellConfirmed : isEvmSellConfirmed;
  ...
  const handleNetworkChange = () => {
    ...
    primaryWalletConnector
      ...
      .then(() => {
        const currentNetworkConfig = primaryWalletConnector.evmNetworks?.find(
        ...
        if (!currentNetworkConfig) {
          ...
          const networkName =
            currentNetworkConfig.vanityName || currentNetworkConfig.name;
          ...
          const nativeCurrency = currentNetworkConfig.nativeCurrency
            ? {
                ...
                symbol: currentNetworkConfig.nativeCurrency.symbol,
                decimals: currentNetworkConfig.nativeCurrency.decimals,
                iconUrl: currentNetworkConfig.nativeCurrency.iconUrl,
              }
            : null;
      ...
      .catch((err: unknown) => {
        console.error("[OnChainTradingPanel] Failed to switch network:", err);
  ...
  const tradeMetrics = useMemo(() => {
    ...
    // Solana: no on-chain quote, use price-based estimate
    if (isSolana) {
      if (tradeType === "buy") {
        const estimatedTokens = price > 0 ? parsedAmount / price : 0;
        return {
          outputAmount: estimatedTokens,
          fees: parsedAmount * 0.02, // 2% fee estimate
  ...
  useEffect(() => {
    if (isBuyFailed && buyTxHash && handledBuyFailedRef.current !== buyTxHash) {
        ...
        if (isSolana) {
          ...
          if (sig) {
            toast.info("Buy order submitted!", {
              ...
              action: {
                label: "View",
                onClick: () => window.open(getTxUrl(sig, 901), "_blank"),
              },
            });
          }
        } else {
  ...
  const handleQuickAmount = (qa: string) => {
    if (tradeType === "buy") {
      ...
      const balance = parseFloat(formatTokenDisplay(tokenBalance as bigint));
      let percentage = 1;
        ...
        <div className="mb-3">
          ...
          <div className="relative">
            ...
            <div className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 text-xs sm:text-sm text-muted-foreground">
              {tradeType === "buy" ? effectiveNativeSymbol : tokenSymbol}
            </div>
          </div>
        </div>
                  ...
                  <span className="font-medium">
                    {isBuyQuoteLoading || isSellQuoteLoading ? (
                      ...
                      <>
                        ...
                        {tradeType === "buy" ? tokenSymbol : effectiveNativeSymbol}
                      </>
                    )}
                  </span>
                </div>
          ...
          {txHash && (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <a
                  href={getTxUrl(txHash, isSolana ? 901 : undefined)}
                  target="_blank"
