...
export function VestingCard({ bondingCurveAddress, symbol }: VestingCardProps) {
	const { vestingInfo, claimableAmount, refetch } =
		useVestingInfo(bondingCurveAddress);
	...
	const isFullyClaimed =
		remaining === 0n && (!claimableAmount || claimableAmount === 0n);
	if (isFullyClaimed) {
		return null;
	}
				...
				<div className="grid grid-cols-2 gap-4">
					<div className="p-3 bg-muted/50 rounded-lg">
						...
						<p className="font-mono font-medium">
							{formatNumber(parseFloat(formatEther(remaining)))}{" "}
							{symbol}
						</p>
					</div>
					...
					{justClaimed && isConfirmed ? (
						...
						>
							<Button
								...
								className="w-full"
								variant={
									hasClaimable && isVestingComplete
										? "default"
										: "secondary"
								}
							>
