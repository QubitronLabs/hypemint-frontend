...
function calculateBollingerBands(data: Candle[], period: number = 20, stdDev: number = 2): { upper: (number | null)[]; middle: (number | null)[]; lower: (number | null)[] } {
...
export function AdvancedPriceChart({
  ...
  useEffect(() => {
    ...
    async function fetchCandles() {
      ...
      try {
        ...
        const res = await fetch(`${apiUrl}/api/v1/charts/${tokenId}?interval=${intervalMap[timeRange]}&limit=500`);
        ...
        for (const d of rawData) {
          ...
          processed.push({ time, open: o, high: h, low: l, close: c, volume: v });
        }
  ...
  const getChartMetrics = useCallback(() => {
    ...
    if (pMin === pMax) { pMin *= 0.99; pMax *= 1.01; }
  ...
  const getPriceY = useCallback((price: number, priceMin: number, priceMax: number, priceHeight: number) => {
    const range = priceMax - priceMin || 1;
    return priceHeight - ((price - priceMin) / range) * priceHeight;
  }, []);

  const getIndexAtX = useCallback((x: number) => {
    const spacing = chartState.candleWidth + 2;
    return Math.floor((x - chartState.offsetX) / spacing);
  }, [chartState]);

  const getPriceAtY = useCallback((y: number, priceMin: number, priceMax: number, priceHeight: number) => {
    const range = priceMax - priceMin || 1;
  ...
  const render = useCallback(() => {
    ...
    const { chartWidth, chartHeight, priceHeight, volumeHeight, startIdx, endIdx, priceMin, priceMax, volumeMax } = metrics;
    ...
    if (candles.length === 0) {
      ...
      ctx.fillText(isLoading ? "Loading..." : "No data", dims.w / 2, dims.h / 2);
      return;
    }
    ...
    for (let i = startIdx; i < endIdx; i++) {
      ...
      ctx.fillStyle = c.close >= c.open ? COLORS.bullishAlpha : COLORS.bearishAlpha;
      ctx.fillRect(
    ...
    if (candles.length > 0) {
      ...
      const color = last.close >= last.open ? COLORS.bullish : COLORS.bearish;
    ...
    for (const d of drawings) {
      ...
      if (d.type === "hline" && d.points[0]?.price !== undefined) {
        const y = getPriceY(d.points[0].price, priceMin, priceMax, priceHeight);
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.lineTo(chartWidth, y);
        ctx.stroke();
        ...
        const x = getCandleX(d.points[0].candleIdx || getIndexAtX(d.points[0].x));
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, chartHeight);
        ctx.stroke();
        ...
        const price1 = d.points[0].price || getPriceAtY(y1, priceMin, priceMax, priceHeight);
        const price2 = d.points[1].price || getPriceAtY(y2, priceMin, priceMax, priceHeight);
        ...
        for (const level of FIB_LEVELS) {
          ...
          ctx.fillStyle = d.color;
          ctx.fillText(`${(level * 100).toFixed(1)}% - ${formatPrice(price)}`, 5, y - 3);
        }
        ctx.globalAlpha = 1;
        ...
        ctx.beginPath();
        ctx.moveTo(p0.x - (dx / len) * 500, p0.y + offsetY - (dy / len) * 500);
        ctx.lineTo(p1.x + (dx / len) * 500, p1.y + offsetY + (dy / len) * 500);
        ctx.stroke();
        ...
        const p1 = d.points[0];
        const p2 = d.points[1];
        const price1 = p1.price || getPriceAtY(p1.y, priceMin, priceMax, priceHeight);
        const price2 = p2.price || getPriceAtY(p2.y, priceMin, priceMax, priceHeight);
        const priceDiff = price2 - price1;
        const pctChange = (priceDiff / price1) * 100;
        
        // Draw rectangle
        ctx.strokeRect(Math.min(p1.x, p2.x), Math.min(p1.y, p2.y), Math.abs(p2.x - p1.x), Math.abs(p2.y - p1.y));
        ctx.fillStyle = priceDiff >= 0 ? "rgba(34,197,94,0.1)" : "rgba(239,68,68,0.1)";
        ...
        ctx.fillStyle = priceDiff >= 0 ? COLORS.bullish : COLORS.bearish;
        ctx.font = "bold 11px -apple-system, system-ui, sans-serif";
        ctx.textAlign = "left";
        ctx.fillText(`${pctChange >= 0 ? "+" : ""}${pctChange.toFixed(2)}%`, infoX + 8, infoY + 18);
        ctx.fillStyle = COLORS.textLight;
        ctx.font = "10px -apple-system, system-ui, sans-serif";
        ctx.fillText(`${formatPrice(priceDiff)}`, infoX + 8, infoY + 35);
      }
    ...
    if (drawingInProgress) {
      ...
      else if ((drawingInProgress.type === "trendline" || drawingInProgress.type === "ray" || drawingInProgress.type === "extended") && pts.length >= 1) {
        const p2 = pts[1] || mp;
        ctx.beginPath();
      ...
      else if ((drawingInProgress.type === "rect" || drawingInProgress.type === "ellipse" || drawingInProgress.type === "measure") && pts.length >= 1) {
        const p2 = pts[1] || mp;
      ...
      else if (drawingInProgress.type === "fibonacci" && pts.length >= 1) {
        const p2 = pts[1] || mp;
        ctx.beginPath();
        ctx.moveTo(0, pts[0].y);
        ctx.lineTo(chartWidth, pts[0].y);
      ...
      else if ((drawingInProgress.type === "pitchfork" || drawingInProgress.type === "channel")) {
    ...
    if (mousePos.x >= 0 && mousePos.x < chartWidth && mousePos.y >= 0 && mousePos.y < chartHeight) {
      if (activeTool !== "cursor") {
        ...
        // Crosshair labels
        const crossPrice = getPriceAtY(mousePos.y, priceMin, priceMax, priceHeight);
        ...
        ctx.font = "10px -apple-system, system-ui, sans-serif";
        ctx.textAlign = "right";
        ctx.fillText(formatPrice(crossPrice), dims.w - 4, mousePos.y + 4);
        ...
        if (idx >= 0 && idx < candles.length) {
          ...
          ctx.fillStyle = COLORS.priceLine;
          ctx.fillRect(mousePos.x - labelW / 2, chartHeight, labelW, TIME_AXIS_HEIGHT);
          ctx.fillStyle = "#000";
          ctx.textAlign = "center";
    ...
    for (let i = startIdx; i < endIdx; i += timeStep) {
      ...
      if (x >= 0 && x <= chartWidth && candles[i]) {
        ctx.fillText(formatTime(candles[i].time, timeRange), x, chartHeight + 15);
      }
    }
  ...
  }, [dims, candles, chartState, mousePos, drawings, drawingInProgress, activeTool, isLoading, timeRange, indicators, selectedDrawingId, drawingColor, getChartMetrics, getCandleX, getPriceY, getIndexAtX, getPriceAtY]);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const price = getPriceAtY(y, metrics.priceMin, metrics.priceMax, metrics.priceHeight);
    ...
    // Two-click tools
    const twoClickTools: DrawingTool[] = ["trendline", "ray", "extended", "rect", "ellipse", "fibonacci", "measure"];
    // Three-click tools
    ...
    if (twoClickTools.includes(activeTool)) {
      if (!drawingInProgress) {
        ...
        const completed: Drawing = {
          ...drawingInProgress,
          points: [...drawingInProgress.points, { x, y, price, candleIdx }],
        };
  ...
  }, [activeTool, chartState, drawingInProgress, drawingColor, getChartMetrics, getPriceAtY, getIndexAtX]);
  ...
  const handleWheel = useCallback((e: React.WheelEvent) => {
    ...
    setChartState(prev => {
      ...
      const newW = clamp(oldW * factor, MIN_CANDLE_WIDTH, MAX_CANDLE_WIDTH);
  ...
  const zoomIn = useCallback(() => {
    setChartState(prev => ({
      ...prev,
      candleWidth: clamp(prev.candleWidth * 1.3, MIN_CANDLE_WIDTH, MAX_CANDLE_WIDTH),
    }));
  }, []);
  ...
  const zoomOut = useCallback(() => {
    setChartState(prev => ({
      ...prev,
      candleWidth: clamp(prev.candleWidth * 0.7, MIN_CANDLE_WIDTH, MAX_CANDLE_WIDTH),
    }));
  }, []);
  ...
  useTokenPriceUpdates(tokenId, (update) => {
    ...
    setCandles(prev => {
      ...
      const intervalSec = { "1m": 60, "5m": 300, "15m": 900, "1h": 3600, "4h": 14400, "1D": 86400 }[timeRange];
  ...
  const toolGroups = [
    {
      label: "Navigation",
      tools: [
        { id: "cursor" as DrawingTool, icon: MousePointer2, tip: "Pan / Select" },
        { id: "crosshair" as DrawingTool, icon: Crosshair, tip: "Crosshair" },
      ]
    },
    {
      label: "Lines",
      tools: [
        { id: "trendline" as DrawingTool, icon: TrendingUp, tip: "Trend Line" },
        { id: "ray" as DrawingTool, icon: ArrowUpRight, tip: "Ray (infinite one direction)" },
        { id: "extended" as DrawingTool, icon: Slash, tip: "Extended Line (infinite)" },
        { id: "hline" as DrawingTool, icon: Minus, tip: "Horizontal Line" },
        { id: "vline" as DrawingTool, icon: MoveVertical, tip: "Vertical Line" },
      ]
    },
    {
      label: "Shapes",
    ...
    },
    {
      label: "Advanced",
      tools: [
        { id: "fibonacci" as DrawingTool, icon: Activity, tip: "Fibonacci Retracement" },
        { id: "pitchfork" as DrawingTool, icon: GitFork, tip: "Andrews Pitchfork (3 points)" },
        { id: "channel" as DrawingTool, icon: Triangle, tip: "Parallel Channel (3 points)" },
        { id: "measure" as DrawingTool, icon: Ruler, tip: "Price Range / Measure" },
        { id: "text" as DrawingTool, icon: Type, tip: "Text Annotation" },
      ]
    },
  ];

  return (
    <div className={cn("flex flex-col bg-[#0f0f0f] rounded-lg overflow-hidden", className)} style={{ height }}>
      {/* Toolbar */}
      {showToolbar && (
        <div className="flex items-center gap-1 px-2 py-1.5 border-b border-white/5 bg-black/40 flex-shrink-0 flex-wrap">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="bg-zinc-900 border-zinc-700">
              {toolGroups.map((group, gi) => (
                <div key={group.label}>
                  {gi > 0 && <DropdownMenuSeparator className="bg-zinc-700" />}
                  <DropdownMenuLabel className="text-zinc-500 text-[10px]">{group.label}</DropdownMenuLabel>
                  {group.tools.map(({ id, icon: Icon, tip }) => (
                    <DropdownMenuItem
                      key={id}
                      onClick={() => { setActiveTool(id); setDrawingInProgress(null); }}
                      className={cn(
                        "flex items-center gap-2 cursor-pointer",
                        activeTool === id ? "bg-[#22c55e]/20 text-[#22c55e]" : "text-zinc-300"
                      )}
                    >
          ...
          <div className="flex items-center bg-white/5 rounded p-0.5">
            {[
              { id: "cursor" as DrawingTool, icon: MousePointer2 },
              { id: "crosshair" as DrawingTool, icon: Crosshair },
              { id: "trendline" as DrawingTool, icon: TrendingUp },
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="p-1.5 rounded hover:bg-white/10 transition-all" title="Drawing Color">
                <div className="w-4 h-4 rounded-sm border border-white/20" style={{ backgroundColor: drawingColor }} />
              </button>
            </DropdownMenuTrigger>
              ...
              {indicators.map(ind => (
                <DropdownMenuItem
                  ...
                  className={cn(
                    ...
                    ind.enabled ? "text-white" : "text-zinc-400"
                  )}
                >
                  ...
                  {ind.enabled && <span className="text-[#22c55e] text-xs">✓</span>}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          {lastCandle && (
            <div className="flex items-center gap-2 px-2">
              ...
              <span className={cn(
                ...
                priceChange >= 0 ? "text-[#22c55e] bg-[#22c55e]/10" : "text-[#ef4444] bg-[#ef4444]/10"
              )}>
          ...
          <div className="flex items-center gap-0.5">
            <button onClick={zoomIn} title="Zoom In" className="p-1.5 text-zinc-500 hover:text-white hover:bg-white/10 rounded transition-all">
              <Plus className="w-3.5 h-3.5" />
            </button>
            <button onClick={zoomOut} title="Zoom Out" className="p-1.5 text-zinc-500 hover:text-white hover:bg-white/10 rounded transition-all">
              <Minus className="w-3.5 h-3.5" />
            </button>
            <button onClick={fitContent} title="Fit All" className="p-1.5 text-zinc-500 hover:text-white hover:bg-white/10 rounded transition-all">
              <Maximize2 className="w-3.5 h-3.5" />
            </button>
            <button onClick={undoLastDrawing} title="Undo (Ctrl+Z)" className="p-1.5 text-zinc-500 hover:text-white hover:bg-white/10 rounded transition-all">
              <Undo2 className="w-3.5 h-3.5" />
            </button>
            <button onClick={clearDrawings} title="Clear All Drawings" className="p-1.5 text-zinc-500 hover:text-red-400 hover:bg-white/10 rounded transition-all">
      ...
      <div ref={containerRef} className="relative flex-1 min-h-0">
        ...
        {drawingInProgress && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/80 text-zinc-300 text-xs px-3 py-1.5 rounded-full border border-white/10 pointer-events-none">
            {drawingInProgress.type === "pitchfork" || drawingInProgress.type === "channel" 
        ...
        {indicators.some(i => i.enabled) && (
          <div className="absolute top-2 right-20 flex flex-col gap-1 pointer-events-none">
            {indicators.filter(i => i.enabled).map(ind => (
              <div key={ind.type} className="flex items-center gap-1.5 bg-black/60 px-2 py-0.5 rounded text-[10px]">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: ind.color }} />
                <span className="text-zinc-400">{ind.type.toUpperCase()}({ind.period})</span>
              </div>
            ))}
          </div>
        )}
