...
import { getUserProfileByAddress, getUserCreatedTokens, type UserProfileResponse } from '@/lib/api/users';
import { followUser, unfollowUser, getUserFollowers, getUserFollowing } from '@/lib/api/auth';
...
export function useUserProfile(address: string | undefined) {
    ...
    return useQuery<UserProfileResponse>({
        ...
        queryFn: async () => {
            console.log('[useUserProfile] Fetching profile for', address, 'hasAuth:', hasAuth);
...
export function useFollowUser() {
    ...
    return useMutation({
        ...
        onMutate: async (address: string) => {
            ...
            const previousData = queryClient.getQueryData<UserProfileResponse>(queryKey);
        ...
        onMutate: async (address: string) => {
            ...
            if (previousData) {
                queryClient.setQueryData<UserProfileResponse>(queryKey, {
                    ...
                    user: {
                        ...previousData.user,
                        followersCount: Math.max(0, previousData.user.followersCount - 1),
                    },
                });
            }
 ...
 */
export function useUserFollowers(address: string | undefined, page = 1, pageSize = 20) {
    return useQuery({
 ...
 */
export function useUserFollowing(address: string | undefined, page = 1, pageSize = 20) {
    return useQuery({
 ...
 */
export function useUserCreatedTokens(userId: string | undefined, page = 1, pageSize = 20) {
    return useQuery({
