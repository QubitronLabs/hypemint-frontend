...
export function useTokenPriceUpdates(
    ...
    onUpdate: (data: { price: string; priceChange24h: number; marketCap: string }) => void
) {
    const { subscribe, unsubscribe, isConnected } = useWebSocket({
        onMessage: (message) => {
            if (message.channel === `price:${tokenId}` && message.event === 'price_update') {
                onUpdate(message.data as { price: string; priceChange24h: number; marketCap: string });
            }
        },
    });
...
export function useTokenTradeFeed(
    ...
    const { subscribe, unsubscribe, isConnected } = useWebSocket({
        onMessage: (message) => {
            if (message.channel === `trades:${tokenId}` && message.event === 'trade_executed') {
...
export function useUserNotifications(
    ...
    const { subscribe, unsubscribe, isConnected } = useWebSocket({
        onMessage: (message) => {
            if (message.channel === `user:${userId}` && message.event === 'notification') {
...
export function useTrendingUpdates(
    ...
    const { subscribe, unsubscribe, isConnected } = useWebSocket({
        onMessage: (message) => {
            if (message.channel === 'global:trending' && message.event === 'trending_update') {
